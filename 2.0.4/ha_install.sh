...
echo "Dowloading HAProxy 2.0.4"
curl http://www.haproxy.org/download/2.0/src/haproxy-2.0.4.tar.gz > haproxy-2.0.4.tar.gz
...
make PREFIX=/opt/haproxy-2.0.4 install
...
[Unit]
Description=HAProxy 2.0.4
After=syslog.target network.target

[Service]
Type=notify
EnvironmentFile=/etc/sysconfig/haproxy
ExecStart=/opt/haproxy-2.0.4/sbin/haproxy -f $CONFIG_FILE -p $PID_FILE $CLI_OPTIONS
