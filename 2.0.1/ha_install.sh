...
echo "Dowloading HAProxy 2.0.1"
curl http://www.haproxy.org/download/2.0/src/haproxy-2.0.1.tar.gz > haproxy-2.0.1.tar.gz

echo "Unpacking Lua"
tar xf lua-5.3.5.tar.gz
echo "Unpacking HAProxy"
tar xf haproxy-2.0.1.tar.gz
...
make PREFIX=/opt/haproxy-2.0.1 install
...
[Unit]
Description=HAProxy 2.0.1
After=syslog.target network.target

[Service]
Type=notify
EnvironmentFile=/etc/sysconfig/haproxy
ExecStart=/opt/haproxy-2.0.1/sbin/haproxy -f $CONFIG_FILE -p $PID_FILE $CLI_OPTIONS
