...
curl http://www.haproxy.org/download/2.0/src/haproxy-2.0.2.tar.gz > haproxy-2.0.2.tar.gz

tar xf lua-5.3.5.tar.gz
tar xf haproxy-2.0.2.tar.gz
...
make PREFIX=/opt/haproxy-2.0.2 install
...
[Unit]
Description=HAProxy 2.0.2
After=syslog.target network.target

[Service]
Type=notify
EnvironmentFile=/etc/sysconfig/haproxy
ExecStart=/opt/haproxy-2.0.2/sbin/haproxy -f $CONFIG_FILE -p $PID_FILE $CLI_OPTIONS
